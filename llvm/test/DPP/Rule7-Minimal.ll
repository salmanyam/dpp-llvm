; RUN: opt -S -passes="print<dpp-local-rule7>" -disable-output < %s 2>&1 | FileCheck %s --check-prefixes=CHECK,LOCAL
; RUN: opt -S -passes="print-dpp-global-rule7" -disable-output < %s 2>&1 | FileCheck %s --check-prefixes=CHECK,GLOBAL
; XFAIL: *
